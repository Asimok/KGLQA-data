...
if __name__ == '__main__':
    ...
    for phase in PHASES:
        for dataset_type in ['middle', 'high']:
            ...
            save_path = f'/data0/maqi/KGLQA-data/datasets/RACE/LangChain/random_select'
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            dataset = read_data(data_path)
            process_dataset = process(dataset)
            save_data(dataset_=process_dataset, save_path_=save_path + f'/{dataset_type}_{phase}.jsonl')
