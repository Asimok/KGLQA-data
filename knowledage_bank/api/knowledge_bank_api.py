...
if __name__ == '__main__':
    ...
    captions_zh = Captions(url="http://219.216.64.231:7036/get_captions", tokenizer=tokenizer_zh, language='zh', max_seq_length=200)
    captions_en = Captions(url="http://219.216.64.231:7036/get_captions", tokenizer=tokenizer_en, language='en', max_seq_length=200)
    ...
    def test():
        ...
        gold_label = 'B'
        chunks, chunk_captions = get_caption(captions_zh, context, caption_max_seq_length_=250)
        context_data, contexts_idx, captions_data, captions_idx = get_caption_and_rel(Retriever_zh, scorer_zh, query, options, context_data=context, captions_sents=chunk_captions, max_word_count=300)
        print()
    ...
    def get_caption_and_rel_en_api():
        ...
        context_data, contexts_idx, captions_data, captions_idx = get_caption_and_rel(Retriever_en, scorer_en, query, deal_options, context_data=context_data, caption_data=caption_data, max_word_count=max_word_count)
        return flask.jsonify({"context_data": context_data, "contexts_idx": contexts_idx, "captions_data": captions_data, "captions_idx": captions_idx})
