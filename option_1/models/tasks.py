...
class CosmosQATask(Task):
    ...
    def get_datasets(self) -> dict:
        return datasets.load_from_disk("/data0/maqi/KGLQA-data/datasets/cosmos_qa")
...
class CustomJSONLTask(Task):
    ...
    def get_datasets(self) -> dict:
        ...
        for phase in phases:
            ...
            cache_path = os.path.join(self.base_path, f"cache/{phase}")
            if os.path.exists(cache_path):
                print(f"Loading {phase} dataset from cache...")
                dataset_dict[phase] = datasets.load_from_disk(cache_path)
            else:
                print(f"Generating {phase} dataset cache...")
                ...
                dataset_dict[phase] = datasets.load_dataset(
                    ...
                    num_proc=64,
                    cache_dir='/data0/maqi/KGLQA-data/datasets/cache'
                )["train"]
                # TODO save dataset to disk
                dataset_dict[phase].save_to_disk(cache_path)
