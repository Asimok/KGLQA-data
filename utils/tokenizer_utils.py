...
def tokenize_examples_for_enc_dec_model(examples, tokenizer, max_seq_length: int,
    ...
    for i in range(len(examples[option_keys[0]])):
        ...
        input_str = f"question: {examples['query'][i]} options: {all_options} context: {examples['context'][i]} </s>"
        target_str = f"{examples[option_keys[examples['label'][i]]][i]}"
...
def tokenize_examples_for_mc_lm_model(examples, tokenizer, max_seq_length: int,
                                      padding_strategy: PaddingStrategy,
                                      truncation_strategy: TruncationStrategy, is_split_into_words: bool):
    """
    ...
    for option_key in option_keys:
        ...
        # input_part2 = [i[1:] for i in input_part2]
